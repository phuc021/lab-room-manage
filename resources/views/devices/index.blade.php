			...
			<tbody>
				...
				<tr @if($i% 2 == 0) class="old" @else class="even" @endif>
					...
					<td id="button-option-edit-device">
						<button title="Edit" type="submit" value="Edit"><a class="ct" href="{{ url('devices') }}/{{$device->id}}/edit">Edit</a></button></td>
					<td id="button-option-del-device">
						<form action="{{ url('devices/'.$device->id) }}" method="POST">
			 				@csrf
							{{ method_field('DELETE') }}
							<button class="pd" type="submit" value="Delete"  title="Delete"  onclick="return confirm('bạn có muốn xóa?');">Delete</button>
						</form>
					</td>
				</tr>
				@endforeach
